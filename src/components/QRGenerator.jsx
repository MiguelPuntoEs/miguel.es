...
function FrameThumb({ id, color }) {
  const c = color || "#d95f02";
  const qr = <rect x="9" y="9" width="26" height="26" fill="#d6d3d1" rx="2" />;
  switch (id) {
    case "none":
      return (
        <svg viewBox="0 0 44 44" className="w-9 h-9">
          <rect x="3" y="3" width="38" height="38" fill="#f5f5f4" rx="3" />
          {qr}
        </svg>
      );
    case "border":
    ...
    case "label-top":
      return (
        <svg viewBox="0 0 44 54" className="w-9 h-11">
          ...
          <rect x="9" y="21" width="26" height="24" fill="#d6d3d1" rx="2" />
        </svg>
      );
    case "banner":
      return (
...
const INPUT = "w-full rounded-lg border border-stone-300 bg-white px-3 py-2 text-sm text-stone-900 placeholder:text-stone-400 focus:border-accent focus:ring-1 focus:ring-accent focus:outline-none";
...
function PhoneInput({ label = "Phone number", value, dialCode, onChangePhone, onChangeDialCode }) {
  ...
  return (
    ...
    >
      <span className={LABEL}>{label}</span>
      <div className="relative flex items-center rounded-lg border border-stone-300 bg-white focus-within:ring-1 focus-within:ring-accent focus-within:border-accent">
        <button
          type="button"
          ...
          className="flex items-center gap-1 pl-3 pr-1 py-2 text-sm hover:bg-stone-50 shrink-0 rounded-l-lg"
        >
          <span>{selected.flag}</span>
          <span className="text-stone-500 text-xs">{selected.dial}</span>
          <svg className="w-3 h-3 text-stone-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
        </button>
        <input
          className="w-full bg-transparent py-2 pr-3 pl-1 text-sm text-stone-900 placeholder:text-stone-400 focus:outline-none rounded-r-lg"
          type="tel"
        ...
        />
        {open && (
          <div className="absolute top-full left-0 mt-1 w-56 max-h-48 overflow-y-auto rounded-lg border border-stone-200 bg-white shadow-lg z-10">
            <div className="sticky top-0 bg-white p-1.5 border-b border-stone-100">
              <input
                className="w-full rounded border border-stone-200 px-2 py-1 text-xs placeholder:text-stone-400 focus:outline-none focus:border-accent"
            ...
            {filtered.map((c) => (
              <button
                key={c.name}
                type="button"
                className={`flex w-full items-center gap-2 px-3 py-1.5 text-xs hover:bg-stone-50 ${c.dial === dialCode ? "bg-accent/5" : ""}`}
                onClick={() => { onChangeDialCode(c.dial); setOpen(false); setSearch(""); }}
              >
                <span>{c.flag}</span>
                <span className="text-stone-500">{c.dial}</span>
...
function UrlInput({ value, onChange }) {
  ...
  const borderClass =
    valid === null ? "border-stone-300" : valid ? "border-green-500" : "border-red-400";

  return (
    <label className="block">
      <span className={LABEL}>URL</span>
      <div className={`flex items-center rounded-lg border bg-white overflow-hidden focus-within:ring-1 focus-within:ring-accent focus-within:border-accent ${borderClass}`}>
        {!hasProtocol && value && (
          <span className="pl-3 text-sm text-stone-400 select-none whitespace-nowrap">https://</span>
        )}
        <input
          className={`w-full bg-transparent py-2 text-sm text-stone-900 placeholder:text-stone-400 focus:outline-none ${!hasProtocol && value ? "pl-0 pr-3" : "px-3"}`}
          type="text"
      ...
      return (
        <div className="space-y-3">
          ...
          <label className="flex items-center gap-2 text-sm text-stone-600">
            <input type="checkbox" checked={data.hidden || false} onChange={set("hidden")} className="rounded border-stone-300" />
            Hidden network
          </label>
        </div>
      );
...
export default function QRGenerator({ defaultType = "url", showTypeSelector = true }) {
  ...
  const [frameColor, setFrameColor] = useState("#d95f02");
    ...
    <div className="mx-auto max-w-4xl">
        ...
        <div className="flex flex-col gap-2 mb-8">
          {QR_TYPE_GROUPS.map((group) => (
            <div key={group} className="flex items-center gap-2">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-stone-500 w-16 shrink-0 text-right">{group}</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="space-y-4">
          ...
          {!logoConfig && (
            <div className="pt-3 border-t border-stone-200">
              <p className="text-xs font-medium text-stone-600 mb-2">Colors</p>
          ...
          {/* Frame option */}
          <div className="pt-3 border-t border-stone-200">
            <p className="text-xs font-medium text-stone-600 mb-2">Frame</p>
            <div className="grid grid-cols-4 gap-2">
              {FRAME_STYLES.map((s) => (
                ...
                >
                  ...
                  <span className="text-[10px] text-stone-600 leading-tight text-center">{s.label}</span>
                </button>
              ))}
            </div>
            {frameStyle !== "none" && (
              <div className="mt-3 flex flex-wrap gap-4">
                <label className="flex items-center gap-2 text-xs text-stone-600">
                  <input
                    ...
                    disabled={!!logoConfig}
                    className="h-8 w-8 rounded border border-stone-300 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
                  />
                  Frame color{logoConfig && <span className="text-stone-400">(matched to brand)</span>}
                </label>
              </div>
            )}
        ...
        <div className="flex flex-col items-center gap-4">
          <div className="rounded-xl border border-stone-200 bg-white p-4 shadow-sm flex items-center justify-center min-h-[17rem]">
            {qrDataUrl ? (
              <img src={qrDataUrl} alt="QR Code" className="max-w-[16rem] max-h-[22rem] w-auto h-auto" />
            ) : (
              <div className="w-64 h-64 flex items-center justify-center text-sm text-stone-400">
          ...
          {qrString && (
            <p className="text-xs text-stone-500 break-all max-w-[280px] text-center">
